// crap warning: you know what to do
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		for (i in 0...keysArray[16].length) keysPressed.push(false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.gfNote = (section.gfSection && (songNotes[1]<8));
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						sustainNote.gfNote = (section.gfSection && (songNotes[1]<8));
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			{
				// hold note functions
				if (strumsBlocked[daNote.noteData] != true && daNote.isSustainNote && dataKeyIsPressed(daNote.noteData % 8) && daNote.canBeHit
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		if (!boyfriend.stunned)
		{
			...
			if(boyfriend.hasMissAnimations) {
				notes.forEachAlive(function(daNote:Note) boyfriend.playAnim(singAnimations[Std.int(Math.abs(daNote.noteData))] + 'miss' + daNote.animSuffix, true));
			}
			vocals.volume = 0;
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		StrumPlayAnim(true, Std.int(Math.abs(note.noteData)) % 8, time);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(cpuControlled) {
				...
				if(spr != null)	
				{
					if (!cpuControlled) spr.playAnim('confirm', true);
				}
			}
			note.wasGoodHit = true;
			vocals.volume = 1;
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		{
			hue = ClientPrefs.arrowHSV[data % 8][0] / 360;
			sat = ClientPrefs.arrowHSV[data % 8][1] / 100;
			brt = ClientPrefs.arrowHSV[data % 8][2] / 100;
			if(note != null) {
