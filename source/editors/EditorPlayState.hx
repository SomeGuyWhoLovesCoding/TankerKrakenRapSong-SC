...
class EditorPlayState extends MusicBeatState
{
			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						...
						if (PlayState.startLocked) daNoteData = Std.int(songNotes[1] % 8);
	...
	override function update(elapsed:Float) {
		...
		if (!PlayState.startLocked) {
			for (i in 0...8) {
				if (PlayState.modchartInitialized && !startingSong) {
					if (PlayState.modchartType1) strumLineNotes.members[i].y = strumLineNotes.members[i].y + Math.sin(FlxG.game.ticks / 1000 + (350 * i)) / (8 - (i / 4));
					if (PlayState.modchartType2) {
			...
			for (i in 0...16) {
				if (PlayState.modchartInitialized && !startingSong) {
					if (PlayState.modchartType1) strumLineNotes.members[i].y = strumLineNotes.members[i].y + Math.sin(FlxG.game.ticks / 1000 + (350 * i)) / (8 - (i / 8));
					if (PlayState.modchartType2) {
	...
	private function keyShit():Void // Original: https://github.com/MeguminBOT/PsychEngine-TheFatAssModCompilation/blob/main/source/PlayState.hx#L3807
	{
		...
		if (PlayState.startLocked) controlHoldArray = [ // 8K Controls
			controls.NOTE_LEFT1,
			controls.NOTE_DOWN1,
			controls.NOTE_UP1,
		...
		if(ClientPrefs.controllerMode) // Controller Input
		{
			...
			if (PlayState.startLocked) controlArray = [ // 8K Controls
	...
	function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		var hue:Float = ClientPrefs.arrowHSV[data % 8][0] / 360;
		var sat:Float = ClientPrefs.arrowHSV[data % 8][1] / 100;
		var brt:Float = ClientPrefs.arrowHSV[data % 8][2] / 100;
		if(note != null) {
