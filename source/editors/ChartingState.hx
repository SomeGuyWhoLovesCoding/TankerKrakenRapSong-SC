...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		infoTxt = new FlxText(975, 32, 0, "", 16);
		...
		add(new FlxButton(10, FlxG.height - 30, "Autosave", function() { /*openChartingMenuSubstate();*/}));
		zoomTxt = new FlxText(96, FlxG.height - 31, 0, "Zoom: 1 / 1", 16);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.x > gridBG.x
			...
			&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom] && !FlxG.keys.pressed.CONTROL)
		{
			...
			if (FlxG.mouse.overlaps(curRenderedNotes))
			{
				curRenderedNotes.forEachAlive(function(note:Note)
				{
					if (FlxG.mouse.overlaps(note))
					{
						...
						else if (FlxG.keys.pressed.ALT && !FlxG.keys.pressed.CONTROL)
						{
							selectNote(note);
							curSelectedNote[3] = noteTypeIntMap.get(currentType);
							updateGrid();
		...
		if (FlxG.keys.pressed.CONTROL) {
			if (controls.NOTE_LEFT_P) addNote(getStrumTime(FlxG.sound.music.time), 0);
			if (controls.NOTE_DOWN_P) addNote(getStrumTime(FlxG.sound.music.time), 1);
			if (controls.NOTE_UP_P) addNote(getStrumTime(FlxG.sound.music.time), 2);
			if (controls.NOTE_RIGHT_P) addNote(getStrumTime(FlxG.sound.music.time), 3);
		}
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE && !FlxG.keys.pressed.CONTROL)
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new editors.EditorPlayState(sectionStartTime()));
			}
			if (FlxG.keys.justPressed.ENTER && !FlxG.keys.pressed.CONTROL)
			{
				autosaveSong();
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN && !FlxG.keys.pressed.CONTROL)
				{
					FlxG.sound.music.pause();
					updateCurStep();
					var time:Float = FlxG.sound.music.time;
			...
			if(vortex && !blockInput){
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN && !FlxG.keys.pressed.CONTROL)
				{
					...
					var snap:Float = quantization / 4;
					var increase:Float = 1 / snap;
					if (FlxG.keys.pressed.UP && !FlxG.keys.pressed.CONTROL)
					{
						var fuck:Float = CoolUtil.quantize(beat, snap) - increase;
						feces = Conductor.beatToSeconds(fuck);
					}
					if (FlxG.keys.pressed.DOWN && !FlxG.keys.pressed.CONTROL) {
						var fuck:Float = CoolUtil.quantize(beat, snap) + increase;
						feces = Conductor.beatToSeconds(fuck);
					}
			...
			if (FlxG.keys.justPressed.A && !FlxG.keys.pressed.CONTROL) {
				if(curSec <= 0) {
		...
		} else if (FlxG.keys.justPressed.ENTER && !FlxG.keys.pressed.CONTROL) {
		...
		if (!holdingShift && pressedLB || holdingShift && holdingLB && !FlxG.keys.pressed.CONTROL)
			playbackSpeed -= 0.01;
		if (!holdingShift && pressedRB || holdingShift && holdingRB && !FlxG.keys.pressed.CONTROL)
			playbackSpeed += 0.01;
		if (FlxG.keys.pressed.ALT && (pressedLB || pressedRB || holdingLB || holdingRB) && !FlxG.keys.pressed.CONTROL)
			playbackSpeed = 1;
		//
