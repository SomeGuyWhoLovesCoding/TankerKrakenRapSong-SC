// From: https://github.com/tposejank/FNF-PsychEngine/blob/extra-keys/source/editors/ChartingState.hx
...
class ChartingState8K extends MusicBeatState
{
	...
	[
		...
		['Screen Shake', "Value 1: Camera shake\nValue 2: HUD shake\n\nEvery value works as the following example: \"1, 0.05, XY\".\nThe first number (1) is the duration.\nThe second number (0.05) is the intensity.\n The third number (XY) is the style.\n(If it's blank, the axes default to XY.)"],
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Chart Editor", StringTools.replace(_song.song, '-', ' ') + " (" + CoolUtil.difficultyString() + ")");
		#end
		...
		leftIcon.setPosition(GRID_SIZE * 3.05, -100);
		rightIcon.setPosition(GRID_SIZE + 455, -100);
		...
		for (i in 0...(8 * 2)){
			var note:StrumNote = new StrumNote(GRID_SIZE * (i+1), strumLine.y, i % 8, 0);
		...
		\n[ / ] - Change Song Playback Rate (SHIFT to go Faster)
		\nALT + [ / ] - Reset Song Playback Rate
		...
		for (i in 0...tipTextArray.length) {
			var tipText:FlxText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 8, 0, tipTextArray[i], 15);
			tipText.y += i * 12;
	...
	function addSongUI():Void
	{
		...
		var player4DropDown = new FlxUIDropDownMenuCustom(player1DropDown.x + 140, player1DropDown.y + 80, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var player3DropDown = new FlxUIDropDownMenuCustom(player1DropDown.x + 140, player1DropDown.y + 40, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		tab_group_song.add(new FlxText(player3DropDown.x, player3DropDown.y - 15, 0, 'Player 3 (Optional):'));
		tab_group_song.add(new FlxText(player4DropDown.x, player4DropDown.y - 15, 0, 'Player 4 (Optional):'));
	...
	override function update(elapsed:Float)
	{
		...
		var playedSound:Array<Bool> = [false, false, false, false, false, false, false, false]; //Prevents ouchy GF sex sounds
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData;
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 8;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					var noteDataToCheck:Int = note.noteData % 8;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 8;
	...
	function reloadGridLayer() {
		...
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * 8 * 2, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * 8 * 2, leHeight + Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * 8 * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 8)).makeGraphic(2, leHeight, FlxColor.BLACK);
		...
		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * 8 * 2), 4);
		add(strumLine);
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			waveformSprite.makeGraphic(Std.int(GRID_SIZE * (8 * 2)), Std.int(gridBG.height), 0x00FFFFFF);
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * (8 * 2));
...
function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[1] > 7) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % 8, null, null, true);
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += 8;
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	public function doANoteThing(cs, d, style){
		...
		if(strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1,strumLine.y+1)) && note.noteData == d%8)
				{
					//trace('tryin to delete note...');
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if(noteData > -1)
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, noteData % (8 * 2), noteSus, noteTypeIntMap.get(daType)]);
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + 8) % (8 * 2), noteSus, noteTypeIntMap.get(daType)]);
		}
